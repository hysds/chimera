...
def validate(sf_context):
    ...
    files = [os.path.basename(f) for f in context['job_specification']['params'][0]['value']]
...
def validate_checksum(filepath, checksum):
    ...
    if checksum != observed_checksum:
        raise ValueError("Invalid checksum. observed:{0}, submitted:{1}".format(observed_checksum, checksum))
