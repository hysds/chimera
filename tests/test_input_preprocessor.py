...
if __name__ == "__main__":
    ...
    context = simplejson.load(open("test-files/L0B_context.json"))
    # ipp_def_filepath = "../nisar_chimera/configs/precondition_definition.yaml"
    chimera_config = (
        "/Users/mcayanan/git/nisar-pcm/nisar_chimera/configs/chimera_config.yaml"
    )
    pge_config = (
        "/Users/mcayanan/git/nisar-pcm/nisar_chimera/configs/pge_configs/PGE_L0B.yaml"
    )
    ...
    for context, pge_config in test_configs:
        payload = ipp.process(
            sf_context=context,
            chimera_config_file=chimera_config,
            pge_config_filepath=pge_config,
