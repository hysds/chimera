...
def submit_pge_job(
    ...
    if not module_path:
        raise RuntimeError(
            "'module_path' must be defined in the 'job_submitter' section of the "
            "Chimera Config file '{}'".format(chimera_config_file)
        )
    class_name = chimera_config.get("job_submitter", {}).get("class_name", None)
    if not class_name:
        raise RuntimeError(
            "'class_name' must be defined in the 'job_submitter' section of the Chimera "
    ...
    if not issubclass(cls, PgeJobSubmitter):
        raise RuntimeError(
            f"Class must be a subclass of {PgeJobSubmitter.__name__}: {cls.__name__}"
        )
    cls_object = cls(
